...
function AchIcon({ name, earned }) {
  ...
  return (
    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${earned ? "bg-[#F6EFDA]" : "bg-[#F0EBE1] opacity-50"}`}>
      <Icon className={`w-6 h-6 ${earned ? "text-[#B0862A]" : "text-[#8A94A6]"}`} />
    </div>
  );
}
  ...
  return (
    <div className="animate-fade-up max-w-4xl">
      ...
      <div className="ls-card p-6 mb-6 flex items-center gap-4">
        <div className="w-16 h-16 rounded-2xl bg-[#C9A227] flex items-center justify-center text-[#1E2A4A] font-display font-bold text-2xl">
      ...
      <div className="ls-card p-6 mb-6">
        ...
        <div className="grid sm:grid-cols-2 gap-5">
          ...
          <div>
            <label className="text-sm font-medium text-[#1E2A4A]">Целевой балл: <span className="text-[#B0862A] font-bold">{form.target_score}</span></label>
            <input data-testid="profile-target" type="range" min="40" max="100" value={form.target_score} onChange={(e) => setForm((f) => ({ ...f, target_score: +e.target.value }))} className="w-full mt-3 accent-[#B0862A]" />
          </div>
          <div>
            <label className="text-sm font-medium text-[#1E2A4A]">Время в день: <span className="text-[#B0862A] font-bold">{form.daily_minutes} мин</span></label>
            <input type="range" min="15" max="240" step="15" value={form.daily_minutes} onChange={(e) => setForm((f) => ({ ...f, daily_minutes: +e.target.value }))} className="w-full mt-3 accent-[#B0862A]" />
          </div>
        </div>
          ...
          <div className="grid sm:grid-cols-3 gap-2 mt-2">
            {subjects.map((s) => (
              <button key={s.id} onClick={() => toggleSubject(s.id)} data-testid={`profile-subject-${s.id}`}
                className={`flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm border-2 text-left ${form.subjects.includes(s.id) ? "border-[#B0862A] bg-[#F6EFDA]" : "border-[#E5DEC9] bg-[#FAF8F3]"}`}>
                <div className="w-4 h-4 rounded flex items-center justify-center" style={{ background: form.subjects.includes(s.id) ? "#B0862A" : "#E5DEC9" }}>
        ...
        <label className="flex items-center gap-3 mt-5 cursor-pointer">
          <input type="checkbox" checked={form.notifications_enabled} onChange={(e) => setForm((f) => ({ ...f, notifications_enabled: e.target.checked }))} className="w-4 h-4 accent-[#B0862A]" data-testid="profile-notifications" />
      ...
      <div className="ls-card p-6">
        <div className="flex items-center gap-2 mb-5">
          <Award className="w-5 h-5 text-[#B0862A]" />
